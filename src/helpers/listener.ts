...
import { NormalizedEvent } from '../typings/events'
...
const addPointerCoords = (
    ...
    return Object.assign(evt, normalizeWindowPosition({x: clientX, y: clientY}, container, ctx))
}
...
export class InteractionListener {
    ...
    constructor(container: HTMLDivElement, eventHandler: EventHandler, ctx: OffscreenContext) {
    ...
    init() {
        if (supportsPointer) {
            this.container.addEventListener('pointerdown', this.onPointerDown, addEventOptions)
            this.container.addEventListener('pointermove', this.onPointerMove, addEventOptions)
        } else if (supportsTouch) {
            this.container.addEventListener('touchstart', this.onTouchStart, addEventOptions)
            this.container.addEventListener('touchmove', this.onTouchMove, addEventOptions)
        } else {
            this.container.addEventListener('mousedown', this.onMouseDown, addEventOptions)
            this.container.addEventListener('mousemove', this.onMouseMove, addEventOptions)
        }
    ...
    removeListeners() {
        ...
        if (supportsPointer) {
            this.container.removeEventListener('pointerdown', this.onPointerDown, addEventOptions)
            this.container.removeEventListener('pointermove', this.onPointerMove, addEventOptions)
        } else if (supportsTouch) {
            this.container.removeEventListener('touchstart', this.onTouchStart, addEventOptions)
            this.container.removeEventListener('touchmove', this.onTouchMove, addEventOptions)
        } else {
            this.container.removeEventListener('mousedown', this.onMouseDown, addEventOptions)
            this.container.removeEventListener('mousemove', this.onMouseMove, addEventOptions)
        }
        ...
        document.removeEventListener('pointermove', this.onPointerMove, addEventOptions)
        document.removeEventListener('pointerup', this.onPointerUp, addEventOptions)
        document.removeEventListener('pointercancel', this.onPointerCancel, addEventOptions)

        document.removeEventListener('mousemove', this.onMouseMove, addEventOptions)
        document.removeEventListener('mouseup', this.onMouseUp, addEventOptions)

        document.removeEventListener('touchmove', this.onTouchMove, addEventOptions)
        document.removeEventListener('touchend', this.onTouchEnd, addEventOptions)
        document.removeEventListener('touchcancel', this.onTouchCancel, addEventOptions)
        // @ts-ignore
        document.removeEventListener('touchforcechange', this.onTouchForceChange, addEventOptions)
        document.removeEventListener('selectstart', this.onSelectionStart, addEventOptions)
    ...
    onTouchStart = (evt: TouchEvent) => {
        for (let i = 0; i < evt.changedTouches.length; i++) {
            ...
            this._bindDownEvents(normalizedEvent, () => {
                this.container.removeEventListener('touchmove', this.onTouchMove, addEventOptions)
                document.addEventListener('touchmove', this.onTouchMove, addEventOptions)
                document.addEventListener('touchend', this.onTouchEnd, addEventOptions)
                document.addEventListener('touchcancel', this.onTouchCancel, addEventOptions)
                if (canReceiveTouchForceChangeEvent()) {
                    // @ts-ignore
    ...
    onPointerDown = (evt: PointerEvent) => {
        const normalizedEvent = normalizePointerEvent(this.container, evt, this.ctx)
        this.eventHandler.onDown(normalizedEvent)
        this._bindDownEvents(normalizedEvent, () => {
            this.container.removeEventListener('pointermove', this.onPointerMove, addEventOptions)
            document.addEventListener('pointermove', this.onPointerMove, addEventOptions)
            document.addEventListener('pointerup', this.onPointerUp, addEventOptions)
            document.addEventListener('pointercancel', this.onPointerCancel, addEventOptions)
        })
    }
    ...
    _bindDownEvents(normalizedEvent: NormalizedEvent, cb: () => void) {
        if (!this._downEventsBinded) {
            if (normalizedEvent.shiftKey) {
                ...
                document.addEventListener('selectstart', this.onSelectionStart, addEventOptions)
            }
            cb()
            this._downEventsBinded = 0
        }
